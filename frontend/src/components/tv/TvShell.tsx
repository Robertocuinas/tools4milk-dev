...
export function TvShell({
  ...
  return (
    <div className="flex min-h-screen flex-col bg-tv-bg font-body text-tv-text">
      {/* ── TV Header ── */}
      <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-tv-border bg-tv-surface px-4 py-4 md:px-8">
        <div className="flex min-w-0 items-center gap-3 md:gap-4">
          ...
          <div>
            <div className="truncate font-heading text-lg font-bold leading-tight text-tv-text">{title}</div>
            {subtitle && <div className="text-xs text-tv-dim">{subtitle}</div>}
          </div>
        </div>

        <div className="flex min-w-0 flex-wrap items-center justify-end gap-3 md:gap-5">
